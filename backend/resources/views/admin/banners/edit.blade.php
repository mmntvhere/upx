...
@section('breadcrumbs')
    <x-admin.breadcrumbs :items="[
        ...
        ['label' => 'Edit Banner: ' . ($banner->title ?: $banner->id)]
    ]" />
@endsection
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow max-w-2xl">
        <form action="{{ route('admin.banners.update', $banner) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="url" name="link" id="link" value="{{ old('link', $banner->link) }}" required 
                       class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded px-3 py-2"
            ...
            <div>
                ...
                <select name="position" id="position" required class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded px-3 py-2">
                    <option value="main" {{ old('position', $banner->position) == 'main' ? 'selected' : '' }}>Main Desktop Banner (Left)</option>
                    <option value="side_top" {{ old('position', $banner->position) == 'side_top' ? 'selected' : '' }}>Small Side Top (Right)</option>
                    <option value="side_bottom" {{ old('position', $banner->position) == 'side_bottom' ? 'selected' : '' }}>Small Side Bottom (Right)</option>
                </select>
            ...
            <div>
                ...
                <img src="{{ asset('storage/' . $banner->image) }}" class="max-w-xs h-auto rounded shadow-sm bg-gray-100" alt="Current Banner">
            </div>

            <!-- Image Upload -->
            <div>
                <label for="image" class="block text-sm font-medium mb-1">Replace Image</label>
